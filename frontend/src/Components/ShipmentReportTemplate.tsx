...
const ShipmentReportTemplate = (props: any) => {
    const [shipment, setShipment] = useState(props.shipment);
    const [inventoriesWithItem, setInventoriesWithItem] = useState(props.inventories);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <Text style={styles.documentTitle}>
          {selectedLanguage === "EN" ? "Client Report" : "Rapport Client"}
        </Text>
        ...
        <View style={styles.shipmentDetails}>
          <View style={styles.detailColumn}>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Shipment ID:" : "ID de l'Expédition:"}</Text>
              <Text style={styles.detailValue}>{shipment.shipmentId}</Text>
            </View>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Shipment Name:" : "Nom de l'Expédition:"}</Text>
              <Text style={styles.detailValue}>{shipment.shipmentName}</Text>
            </View>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Customer's Name:" : "Nom du Client:"}</Text>
              <Text style={styles.detailValue}>{`${shipment.firstName} ${shipment.lastName}`}</Text>
            </View>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Email:" : "Courriel:"}</Text>
              <Text style={styles.detailValue}>{shipment.email}</Text>
            </View>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Pickup Address:" : "Adresse de Ramassage:"}</Text>
              <Text style={styles.detailValue}>{`${shipment.pickupAddress.streetAddress}, ${shipment.pickupAddress.city}, ${shipment.pickupAddress.postalCode}, ${shipment.pickupAddress.country}`}</Text>
            </View>
          </View>
          <View style={styles.detailColumn}>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Phone Number:" : "Numéro de Téléphone:"}</Text>
              <Text style={styles.detailValue}>{shipment.phoneNumber}</Text>
            </View>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Expected Moving Date:" : "Date de Déménagement Prévue:"}</Text>
              <Text style={styles.detailValue}>{shipment.expectedMovingDate}</Text>
            </View>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Approximate Weight:" : "Poids Approximatif:"}</Text>
              <Text style={styles.detailValue}>{shipment.approximateWeight}</Text>
            </View>
            <View style={styles.detailGroup}>
              <Text style={styles.detailLabel}>{selectedLanguage === "EN" ? "Destination Address:" : "Adresse de Destination:"}</Text>
              <Text style={styles.detailValue}>{`${shipment.destinationAddress.streetAddress}, ${shipment.destinationAddress.city}, ${shipment.destinationAddress.postalCode}, ${shipment.destinationAddress.country}`}</Text>
            </View>
          </View>
        </View>
        ...
        {inventoriesWithItem?.map((inventory: Inventory, index: any) => (
          <View key={index} style={styles.inventorySection}>
            <Text style={styles.inventoryHeader}>
              {selectedLanguage === "EN" ? inventory.name : "Nom de l'inventaire"} - <Text style={styles.inventoryStatus}>[{inventory.inventoryStatus}]</Text>
            </Text>
...
export default ShipmentReportTemplate;
