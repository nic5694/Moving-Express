...
function ShipmentReviewerPage() {
  ...
  const acceptQuote = async (quoteId: string) => {
    try {
      ...
      toast.error(selectedLanguage === "EN" ?'Error Accepting Quote' : `Erreur lors de l'acceptation du devis.`, {
        position: 'top-right',
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
          ...
          <div className='flex justify-center'>
            ...
            <div className='bg-white mt-8 w-[80vw] h-[90vh] rounded-lg px-5 py-5'>
              ...
              <div className='mt-5 border-b  border-t'>
                <ShipmentQuoteDetails shipmentQuote={selectedQuote} isEstimatorPage={false} />
              </div>

              <div className="flex flex-row gap-1 py-3">
                  {selectedQuote.quoteStatus !== "DECLINED" ?
                    <div className=' flex flex-row gap-3'>
                      <div><button style={{ fontFamily: 'Bebas Neue, cursive'}} id='acceptBtn' onClick={() => { acceptQuote(selectedQuote.quoteId) }} className='px-2.5 py-1 bg-companyYellow text-white rounded text-sm'>{selectedLanguage === "EN" ? "Accept" : "Accepter"}</button></div>
                      <div><button style={{ fontFamily: 'Bebas Neue, cursive'}} id='declineBtn' onClick={() => { declineQuote(selectedQuote.quoteId) }} className='px-2.5 py-1 bg-gray-400 text-white rounded text-sm'>{selectedLanguage === "EN" ? "Decline" : "Refuser"}</button></div>
                    </div>
                    : <div> </div>
                  }
              </div>
        ...
        <div className='w-[300px] bg-[#2D2D2D] h-[100vh] hidden lg:block py-10 lg:flex justify-center'>
          <div className='flex flex-col text-white'>
            ...
            <div style={{ fontFamily: 'Bebas Neue, cursive', letterSpacing: "1px" }} className='pt-5 pb-5'>
              {selectedLanguage === "EN" ? "Shipment Reviewer" : "Examinateur d'Expédition"}
            </div>
...

                {
                  pendingQuotes.map((quote: any) => (
                    ...
                    <tr id={quote.quoteId} className='text-center text-sm'>
                      ...
                      <td id={`pending-${quote.quoteId}`} className='border px-3 '>
                      {
                        selectedLanguage === "EN" ? 
                        `${quote.quoteStatus}`
                        :
...

                {
                  declinedQuotes.map((quote: any) => (
                    ...
                    <tr name={quote.name} id={quote.quoteId} className='text-center text-sm'>
                      ...
                      {//@ts-ignore
                        <td name={quote.shipmentName} className='border px-3 '><button id={`btn-${quote.quoteId}`} onClick={() => { getQuoteDetails(quote.quoteId) }} style={{ fontFamily: 'Bebas Neue, cursive' }} className="bg-companyYellow text-white py-1 px-10 rounded-sm text-sm">{selectedLanguage === "EN" ? "View" : "Afficher"}</button></td>
                      }</tr>
