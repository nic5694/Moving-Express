...
async function test_case() {
    ...
    const editBtn = await driver.findElement(By.id(`editBtnOption`));
    await editBtn.click();
    await new Promise(resolve => setTimeout(resolve, 1000));

    await new Promise((resolve) => setTimeout(resolve, 3000))

    const inventoryNameInput = await driver.findElement(By.id('inventoryNamePopUp'));
    await inventoryNameInput.clear();
    await inventoryNameInput.sendKeys('New Inventory Name');

    const inventoryDescriptionInput = await driver.findElement(By.id('inventoryDiscriptionPopUp'));
    await inventoryDescriptionInput.clear();
    await inventoryDescriptionInput.sendKeys('New Inventory Description');


    let saveBtn = await driver.findElement(By.id('saveInventoryButton'));
